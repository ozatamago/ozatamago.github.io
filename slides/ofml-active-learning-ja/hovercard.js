/* hovercard.js — shared cross-reference preview (Wikipedia-style).
   Hover a .xref/.cite to preview:
     ...
     - .xref href to any internal page in window.__PAGEMAP__ -> that page summary (fallback)
...
(function () {
  ...
  // Wikipedia-style fallback: any internal page link previews its page summary.
  function pageHtml(a) {
    var P = window.__PAGEMAP__ || {}; var href = a.getAttribute('href'); if (!href) return null;
    var u; try { u = new URL(href, a.baseURI); } catch (e) { return null; }
    if (u.origin !== location.origin) return null;      // external links: no page preview
    var page = u.pathname.split('/').pop() || 'index.html';
    var p = P[page]; if (!p) return null;
    return '<div class="hc-k">' + p.title + '</div><div class="hc-b">' + p.body + '</div>';
  }
    ...
    if (ref && R[ref]) {
      ...
      if (ah) { htmlc = ah; }
        ...
        if (f) {
          ...
          var ph = pageHtml(a); if (!ph) return;
          htmlc = ph;
        }
      }
    } else return;
